 ...
 * Demo state switcher.
 *
 * Rendered as a row *inside* the app header rather than as its own bar above
 * it, so the header reads as one unit instead of a detached browser-chrome
 * strip floating over the product.
 */
  ...
  return (
    <div className="border-t border-line/50">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="tap focusring flex w-full items-center justify-between gap-3 py-2 text-left"
      >
                ...
                >
                  <span
                    className={`mt-0.5 grid h-4 w-4 shrink-0 place-items-center rounded-full border transition-all ${
                      active ? "border-ember bg-ember text-white" : "border-line-2 text-transparent"
                    }`}
                  >
